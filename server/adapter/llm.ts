...
export async function chat(messages: LLMMessage[]): Promise<string> {
  try {
    ...
    if (err.code === 'ECONNREFUSED' || err.message?.includes('fetch failed') || err.cause?.code === 'ECONNREFUSED' || err.name === 'APIConnectionError' || err.name === 'ConnectionError' || err.message?.includes('Connection error')) {
      throw new Error('LLM_UNAVAILABLE: LLM server tidak berjalan di ' + llmConfig.baseUrl);
    }
    if (err.status === 404) {
      throw new Error('LLM_MODEL_NOT_FOUND: Model ' + llmConfig.model + ' tidak ditemukan di LLM server');
    }
...
export async function chatStream(options: LLMStreamOptions): Promise<void> {
  try {
    ...
    if (err.code === 'ECONNREFUSED' || err.message?.includes('fetch failed') || err.cause?.code === 'ECONNREFUSED' || err.name === 'APIConnectionError' || err.name === 'ConnectionError' || err.message?.includes('Connection error')) {
      errorToReport = new Error('LLM_UNAVAILABLE: LLM server tidak berjalan di ' + llmConfig.baseUrl);
    } else if (err.status === 404) {
      errorToReport = new Error('LLM_MODEL_NOT_FOUND: Model ' + llmConfig.model + ' tidak ditemukan di LLM server');
    } else {
